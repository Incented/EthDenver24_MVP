...
import { contributeToMilestoneAction } from "@/data/user/milestones";
...
interface AddMilestoneContributionProps {
  isClaimed?: boolean;
  isClaimer?: boolean;
  milestone_id: string;
}

const AddMilestoneContribution: FC<AddMilestoneContributionProps> = ({ isClaimed, isClaimer, milestone_id }) => {
  ...
  const { mutate } = useToastMutation(
    async (data: ContributionFormSchema) => {
      ...
      return await contributeToMilestoneAction({
    ...
    },
    {
      ...
      onSuccess: (data) => {
        router.push(`/milestones/${milestone_id}`);
...
export default AddMilestoneContribution;
