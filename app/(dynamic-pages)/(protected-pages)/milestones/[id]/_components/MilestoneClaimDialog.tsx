...
import { claimMilestoneAction } from "@/data/user/milestones";
import { useToastMutation } from "@/hooks/useToastMutation";
import { Carrot, Info, Wallet } from "lucide-react";
import { FC, useState } from "react";
import { useForm } from "react-hook-form";
import { carrotDenomination } from "../../../dashboard/(my-dashboard)/data";
import { TokenData } from "./TokenData";

interface MilestoneClaimDialogProps {
  claimStakeAmount: number | null;
  milestone_id: string;
}

const MilestoneClaimDialog: FC<MilestoneClaimDialogProps> = ({ claimStakeAmount, milestone_id }) => {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const { handleSubmit } = useForm();
  const { mutate: claimTask } = useToastMutation(async () => {
    await claimMilestoneAction(milestone_id);
  }, {
    loadingMessage: "Claiming milestone task...",
    successMessage: "Milestone task claimed successfully!",
    errorMessage: "Failed to claim milestone task",
  });
...
export default MilestoneClaimDialog;
