...
import { createGrantApplicationAction, createMilestoneForGrantProject } from "@/data/user/grant-projects";
...
export function CreateGrantApplicationForm({
  ...
  const { mutate } = useToastMutation(
    async (grantProjectData: BasicGrantApplicationFormSchema) => {
  ...
  const { mutate: createMilestoneForGrantMutation } = useToastMutation(
    async (milestoneData: CreateMilestoneForGrantSchema) => {
      const grant_project_id = milestoneData.grant_project_id;
      const title = milestoneData.milestone_title;
      const description = milestoneData.milestone_description;
      const effort = milestoneData.milestone_effort;
      const budget = milestoneData.milestone_budget;

      return await createMilestoneForGrantProject({
        grant_project_id,
        milestone_title: title,
        milestone_description: description,
        milestone_effort: effort,
        milestone_budget: budget,
      });
    },
    {
      loadingMessage: "Creating milestone...",
      errorMessage: "Failed to create milestone",
      successMessage: "Milestone created successfully!",
      onSuccess: (data) => {
        router.push(`/grant-applications/${data.id}`);
      },
    }
  );
  ...
  const onSubmit = async (values: CreateGrantApplicationFormSchema) => {
    ...
    mutate(grantApplicationData);

    // Create the milestones
    values.grant_milestones.forEach((milestone: CreateMilestoneForGrantSchema) => {
      createMilestoneForGrantMutation(milestone);
    });
  };
